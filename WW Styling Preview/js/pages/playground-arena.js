// Page-specific scripts for playground/arena.html
/**
         * Definitionen der verfügbaren Heuristiken.
            ...
            'ultimate': [
                {
                    ...
                    desc: 'Berücksichtigt lokale Boards und Makro-Strategie.',
                    formula: 'Global: ±50, Lokal: ±Regular',
                    fn: HeuristicsLibrary.ultimateTTT
                }
            ],
            'c4_regular': [
                { id: 'c4_std', name: 'Standard Scoring', desc: 'Bewertet Gewinnmöglichkeiten und Zentrum.', formula: 'Lines + Center', fn: HeuristicsLibrary.connect4.evaluate }
            ],
            'c4_3d': [
        ...
        const Arena = {
            adapter: null,  // GameAdapter für einheitliche Schnittstelle
            canvas: null,
            ...
            init() {
                ...
                this.changeGameType();
                // updateKIConfig wird in changeGameType am Ende aufgerufen via reset -> updateKIConfig nicht direkt nötig
            ...
            changeGameType() {
                ...
                // Heuristik Defaults resetten wenn Spieltyp ändert
            ...
            reset() {
                ...
                this.isBattle = false;
                document.getElementById('btnBattle').innerText = "⚔️ Battle";
            ...
            updateKIConfig(player) {
                ...
                // 3. MINIMAX
                    ...
                    if (this.gameType === '3d' || this.gameType === 'ultimate' || this.gameType.startsWith('c4_')) {
                         ...
                         if (depth > limit) warning = '<div class="warn-msg">⚠️ Hohe Tiefe kann langsam sein!</div>';
            ...
            toggleAlphaBeta(player, checked) {
                ...
                this.updateKIConfig(player); // UI Refresh für Warnungs-Logik
            },
            ...
            updateDepth(player, depth) {
                ...
                // Warnung live updaten wenn nötig (hier vereinfacht: Reload der Config)
                 if (this.gameType === '3d' || this.gameType === 'ultimate' || this.gameType.startsWith('c4_')) {
            ...
            getValidMoves() {
                // ✅ Nutze die standardisierte Schnittstelle
            ...
            getValidMovesFrom(gameState) {
                // ✅ Nutze die standardisierte getAllValidMoves() Schnittstelle
                return gameState.getAllValidMoves();
            },

            cloneGame(gameState) {
                // ✅ Nutze die standardisierte clone() Methode
            ...
            async stepSingleGame() {
                ...
                try {
                    ...
                    while(!this.adapter.getState().winner && moveCount < maxMoves) {
                        ...
                        this.updateStatus(`Spiel läuft... Zug ${moveCount + 1}`);

                        const move = await this.getKIMove(gameBoard, p, viz);

                        if (move !== null && move !== undefined) {
                            try {
                                // ✅ makeMove akzeptiert flexibel beide Formate
            ...
            async playBattleGame() {
                ...
                // Startspieler setzen (für Battle auch jedes mal neu prüfen, wichtig für "Zufall")
                const startSel = document.getElementById('startPlayerSelector').value;
                let startPlayer = 1;
                if(startSel === '2') startPlayer = 2;
                else if(startSel === 'random') startPlayer = Math.random() < 0.5 ? 1 : 2;
